...
<form method="get" action="./?menu=productos" style="margin-bottom: 5px;">
	<input type="hidden" name="menu" value="productos" />
	<input type="text" name="q" value="<?php if(isset($_GET['q'])) { echo($_GET['q']); } ?>" class="span4" placeholder="c&oacute;digo o nombre" />
	...
	<select name="c" id="categoria" class="span3">
		...
		<option value="&Uacute;ltimos productos"<?php if(isset($_GET['c']) and ($_GET['c'] == "&Uacute;ltimos productos")) { echo(" selected=\"selected\""); } ?>>&Uacute;ltimos productos</option>
		<option value="Productos m&aacute;s le&iacute;das"<?php if(isset($_GET['c']) and ($_GET['c'] == "Productos m&aacute;s le&iacute;das")) { echo(" selected=\"selected\""); } ?>>Productos m&aacute;s le&iacute;das</option>
...
<div class="tabbable">
	...
	<div class="tab-content">
		...
		<div id="tab2" class="tab-pane">
			<?php
				...
				$registros = $conexion->Query("SELECT * FROM contenidos WHERE (tipo_contenido = 'novedades') and (activo = 1) ORDER BY fecha_alta DESC LIMIT 0,10");
				if($registros) {
					if(!($registros->RecordCount() > 0)) {
						?>
				...
				<div class="carousel-inner">
					<?php
						...
						while(!$registros->EOF) {
							...
							$datosComentarios = $conexion->GetRow("SELECT count(*) as CANTIDAD FROM mensajes WHERE (tipo_mensaje = 'comentarios') and (tipo_cabeza = 'novedades') and (id_cabeza = '".$registros->fields["ID_CONTENIDO"]."') and (activo = 1)");
							...
							if($datos["CANTIDAD"] > 0) {
								for($i=1; $i<=$datos["CANTIDAD"]; $i++) {
									...
									if($i == 1) {									
										...
										$nombre_completo_imagenoriginal = $path_imagenes."contenidos/".$datosArchivo["NOMBRE_ARCHIVO"];
		...
		<div id="tab3" class="tab-pane">
			<?php
				...
				$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and ((click > 0) and (click is not null) and (click <> '')) ORDER BY click DESC");
				if($registros) {
					if(!($registros->RecordCount() > 0)) {
						?>
				<p style="text-align: center;color: #ff0000;"><strong>No se han encontrado coincidencias.</strong></p>
	...
	//	obtengo todas las CATEGORIAS del menu de contenidos
	$registros = $conexion->Query("SELECT CATEGORIA,count(*) as CANTIDAD FROM productos WHERE (activo = 1) and (visible = 1) and ((categoria is not null) and (categoria <> '')) GROUP BY categoria ORDER BY categoria LIMIT 0,10");
