...
$usuarios_sesion  = "superzonagames_xpanel3";
$usuarios_sesion2  = "uszg";
define('XPANEL_SUCURSAL', '1');
...
function convertirTipoMoneda(){
    $multiplo = 1;
    if(($_SESSION['usuario_tipomoneda'] !== 'Bs') && !empty($_SESSION['usuario_tipocambio'])){
        $valor = str_replace(",", ".", $_SESSION['usuario_tipocambio']);
		$valor = preg_replace('/\.(?=.*\.)/', '', $valor);
		$tipo_cambio = floatval($valor) > 0 ? floatval($valor) : 1;

        $multiplo = 1 / $tipo_cambio;
    }
