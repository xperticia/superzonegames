...
$contenido = "";
$archivo = fopen('importarStock_pcdvd.csv', 'r');
$tamano = filesize('importarStock_pcdvd.csv');
...
<tr>
	...
	<td><?php echo(trim($campos[0]."-".$campos[1]." [".$campos[5]."]")); ?></td>
		<?php
		$registros = $conexion->Query("SELECT * FROM productos WHERE (codigo = '".$campos[0]."-PC')");
	...
	<td><?php echo("STOCK: ".$campos[5]); ?></td>
			...
			$conexion->Execute("INSERT productos (ID_PRODUCTO,CODIGO,CATEGORIA,NOMBRE,GENERO,IDIOMA,ACTIVO,CLICK) VALUES(null,'".$campos[0]."-PC"."','".$campos[2]."','".strtoupper($campos[1])."','".$campos[3]."','".$campos[4]."','1','0')");
