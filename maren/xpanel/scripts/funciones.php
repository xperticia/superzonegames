							...
							if($archivos->RecordCount() > 0) {
								...
								while(!$archivos->EOF) {
									...
									if(verificar_url($nombre_completo_documento)) {
										...
										$archivos_adjuntos .= "<li><span style='color: #FF0000;font-weight: bold;'>".$sitio_url.$path_archivos."boletin/".$registros3->fields["NOMBRE_ARCHIVO"]."</span></li>";
									}
									$archivos->MoveNext();
								}
	...
	if (!$record) {   
		...
		<div class="btn-toolbar">
			<div class="btn-group">
				...
				<?php if(($acciones == true) and ($datos_acceso and ($datos_acceso["ELIMINAR"] == 1))) { ?><button id="btEliminarRegistro" class="btn btn-danger" title="Eliminar (Alt+D)" disabled="disabled">Eliminar</button><?php } ?>
			</div>
			<?php 
			//muestro al opcion de RECUPERAR, si tiene permitido ELIMINAR el USUARIO
			if(($acciones == true) and ($datos_acceso and ($datos_acceso["ELIMINAR"] == 1)) and (in_array($_GET['menu'],array(0 =>'ventas',1 => 'servicios',2 => 'pedidos',3 => 'productos')))) { 
				?>
			<div class="btn-group">
				...
				<?php
					...
					for ($i=0; $i<min($reg1+$cantidad_filas, $total); $i++) {
						if($i >= $reg1) {
							...
							?>
						<tr class="unChecked" <?php echo( isset($record->fields['ACTIVO']) && empty($record->fields['ACTIVO']) ? 'style="color: #dc3545 !important;"' : ''); ?>>
							<td><input type="checkbox" id="registro[]" name="registro[]" value="<?php echo($registro); ?>" /></td>
							<?php if($acciones == true) { ?>
							...
							<td <?php if($style_colores != "") { echo("style='".$style_colores."'"); }  ?>>
										...
										if((($campo == "ACTIVO") or ($campo == "USUARIO_ACTIVO")) and ($datos_acceso["MODIFICAR"] == 1)) {						
											if($record->fields[$campo] == "1") {
												echo("<input type=\"checkbox\" name=\"estado\" checked=\"checked\" title=\"".$estados[$record->fields[$campo]]."\" /> ");
											} else {
												if($record->fields[$campo] == null) {
													echo("<input type=\"checkbox\" name=\"estado\" title=\"NO\" /> ");
												} else {
													echo("<input type=\"checkbox\" name=\"estado\" title=\"".$estados[$record->fields[$campo]]."\" /> ");
												}
											}
										} else {
											switch($campo) {
												...
												case "RECIBIDO":
												case "SALDO":
													echo(number_format($record->fields[$campo], $cantidad_decimales, ',', '.'));
													break;
												case "NOMBRE":
													if($_GET['menu'] == "productos") {
														if($datos_acceso["MODIFICAR"] == 1) {
												...
												case "COORDENADAS":
													if($record->fields[$campo] != ""){
														...
														$latitud = $array_datos[0];
														$longitud = $array_datos[1];

														if($_GET['menu'] == "clientes") {
															echo("<a href='https://www.google.com/maps/search/?api=1&query=".$latitud.",".$longitud."' target='_blank' rel='noopener noreferrer'
