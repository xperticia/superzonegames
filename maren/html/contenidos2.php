...
<?php
	...
	if(isset($_GET['accion']) and ($_GET['accion'] == "listar")) {
		if(isset($_GET['c'])) {
			...
			$registros = $conexion->Query("SELECT * FROM contenidos WHERE (activo = 1) and (tipo_contenido = '".$_GET['menu']."') and (categoria = '".$_GET['c']."') ORDER BY fecha_alta DESC,id_contenido DESC");
