	...
	if(isset($_GET['accion']) && ($_GET['accion'] == "listar")) {
		if(isset($_GET['c'])) {
			if($_GET['c'] != "") {
				...
				$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and (upper(categoria) = upper('".$_GET['c']."')) and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY nombre");
				$titulo = "Listado de categor&iacute;a: &quot;".ucfirst($_GET['c'])."&quot;";
				$path .= " &raquo; ".$titulo;
				$url = urlencode($sitio_url."?menu=".$_GET['menu']."&accion=".$_GET['accion']."&c=".$_GET['c']);
			} else {
				$registros = $conexion->Query("SELECT CATEGORIA,count(*) as CANTIDAD FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') GROUP BY categoria ORDER BY categoria");
			...
			if(isset($_GET['g'])) {
				if($_GET['g'] != "") {
					$generoActual = $_GET['g'];
					$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and ( (upper(genero) = upper('".$_GET['g']."')) or (upper(genero) LIKE upper('%".$_GET['g']."%')) ) and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY genero");
				...
				$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY nombre");
...
<tbody>
					...
					foreach($listaGeneros as $key=>$val) {
						$cantidad = 0;
						$datosGenero = $temp->Query("SELECT count(*) as CANTIDAD FROM productos WHERE (activo = 1) and (visible = 1) and ( (upper(genero) LIKE upper('%".$val."%')) and (genero is not null) and (genero <> '') ) and (id_sucursal = '".XPANEL_SUCURSAL."') GROUP BY genero ORDER BY genero");
						if($datosGenero) {
...
<tr>
	...
	<td valign="top">
		...
		<p>
			...
			<?php
			if(($registros->fields["GENERO"] != null) && ($registros->fields["GENERO"] != "")) {
				$contador = 0;
		...
		if(isset($_GET['id'])) {
			...
			$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (id_producto = '".$_GET['id']."') and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY id_producto DESC LIMIT 1");
			if($registros && $registros->RecordCount() > 0) {
				while(!$registros->EOF) {
					...
					?>
	<h1><?php echo("[".$registros->fields["CODIGO"]."] ".stripslashes($registros->fields["NOMBRE"])); ?></h1>
				...
				<?php
					...
					if($registros->fields["SOLOAPEDIDO"] == 1) {
						...
						$datosStock = $temp->Query("SELECT STOCK FROM sucursal_posee_productos WHERE (id_producto = '".$registros->fields["ID_PRODUCTO"]."') and (stock > 0)");
						if($datosStock && ($datosStock->RecordCount() > 0)) { 	echo("EN STOCK"); } else { echo("SIN STOCK"); }
					}
				?>
				</p>
					<?php
	...
	<div class="tabbable">
		...
		<div class="tab-content">
			<div id="tab01" class="tab-pane active" style="min-height: 300px;">
				<div class="row-fluid">
					<div class="span8">
						<?php
						$nombre_completo_imagenoriginal = XPANEL_SITIO_URL_HOST.$path_imagenes."productos/".$registros->fields['FOTO'];
					...
					<div class="span4">
						...
						<tbody>
							...
							<tr>
								...
								<td><?php if(($registros->fields["FECHA_LANZAMIENTO"] != null) && ($registros->fields["FECHA_LANZAMIENTO"] != "0000-00-00")) { echo(formato_fecha($registros->fields["FECHA_LANZAMIENTO"], 1)); } else { echo("NO ESPECIFICADO"); } ?></td>
							</tr>
						</tbody>
						</table>
						<hr />
			...
			<div id="tab02" class="tab-pane" style="min-height: 300px;">
				<?php
			...
			<div id="tab03" class="tab-pane" style="min-height: 300px;">
...
<form method="get" action="./" id="fbuscar" class="form-inline" style="margin-bottom: 5px;">
	...
	if(!isset($_GET['i'])){ 
		...
		if($_GET['i'] > 1){ 
			...
			$_GET['i']++;
			//echo("[".$_GET['i']."]: sumo uno"); 
		} 
	}  
	?>
	<input type="hidden" id="i" name="i" value="<?php echo($_GET['i']); ?>" />

	<div class="row-fluid">
		<div class="span6">
			<div class="input-append" style="width: 98%;">
				<input type="text" name="q" value="<?php if(isset($_GET['q'])) { echo($_GET['q']); } ?>" placeholder="c&oacute;digo o nombre" class="span10" autofocus />
		...
		<?php
			if(isset($_GET['c']) && isset($_GET['ca']) && ($_GET['c'] != $_GET['ca'])) {
				$_GET['g'] = "";
			}
		?>
		<div class="span3">
		...
		<div class="span3">
			...
			$listaGeneros = array();
			$query = "SELECT GENERO FROM productos WHERE (activo = 1) and (visible = 1) and ((genero is not null) and (genero <> '')) and (id_sucursal = '".XPANEL_SUCURSAL."') ";
			if(isset($_GET['c']) != ""){
			...
			<select name="g" id="genero">
				...
				<?php
					foreach($listaGeneros as $key=>$val) {
					?>
				<option value="<?php echo($val); ?>"  <?php if(isset($_GET['g']) && ($val == $_GET['g'])) { echo("selected=\"selected\""); } ?>><?php echo($val); ?></option>
					<?php
					}
				?>
			</select>
...
*/
				$query .= " and ( (upper(codigo) like upper('%".$_GET['q']."%')) or (upper(nombre) like upper('%".$_GET['q']."%')) )"; 
			}
			if(isset($_GET['c']) && ($_GET['c'] != "")) {
				if($_GET['c'] == "Productos m&aacute;s le&iacute;das") {
					$query .= " and ((click > 0) and (click is not null) and (click <> ''))"; 					
					...
					if($_GET['c'] != "&Uacute;ltimos productos") {
						$query .= " and (upper(categoria) = upper('".$_GET['c']."'))";
					}
				}
			}
			if(isset($_GET['g']) && ($_GET['g'] != "")) {
				$query .= " and (upper(genero) like upper('%".$_GET['g']."%'))"; 
			}
			
			if(isset($_GET['c']) && ($_GET['c'] == "Productos m&aacute;s le&iacute;das")) {
				$registros = $conexion->Query("SELECT ID_PRODUCTO,CODIGO,NOMBRE,CATEGORIA,FOTO FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') ".$query." ORDER BY click DESC");
			} else {
				$registros = $conexion->Query("SELECT ID_PRODUCTO,CODIGO,NOMBRE,CATEGORIA,FOTO FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') ".$query." ORDER BY id_producto DESC");
			}
...
/*		$registros = $conexion->Query("SELECT CATEGORIA FROM productos WHERE (activo = 1) and (id_producto = '".$_GET['id']."') GROUP BY categoria ORDER BY categoria");
		while($registros && !$registros->EOF) {
			...
			while($registros2 && !$registros2->EOF) {
	...
	//	obtengo todas las CATEGORIAS del menu de contenidos
	$registros = $conexion->Query("SELECT CATEGORIA,count(*) as CANTIDAD FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') GROUP BY categoria ORDER BY categoria LIMIT 0,10");
	if($registros && $registros->RecordCount() > 0) {
	...
	$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY id_producto DESC LIMIT 0,10");
	if($registros && $registros->RecordCount() > 0) {
		$categorias .= "	<div class=\"btn-group\">\n";
	...
	$registros = $conexion->Query("SELECT * FROM productos WHERE (activo = 1) and (visible = 1) and (click > 0) and (id_sucursal = '".XPANEL_SUCURSAL."') ORDER BY click DESC LIMIT 0,10");
	if($registros && $registros->RecordCount() > 0) {
		$categorias .= "	<div class=\"btn-group\">\n";
